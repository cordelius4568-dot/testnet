...
import { ChogAPI } from 'src/modules/zerion-api/zerion-api.background';
...
async function queryWalletPortfolio(params: Params) {
  return backgroundQueryClient.fetchQuery({
    ...
    queryFn: () => ChogAPI.walletGetPortfolio(params),
    staleTime: 20000,
  });
}
...
async function fetchWalletsMeta({ addresses }: { addresses: string[] }) {
  return backgroundQueryClient.fetchQuery({
    queryKey: ['ChogAPI.getWalletsMeta', addresses],
    queryFn: () => ChogAPI.getWalletsMetaByChunks(addresses),
...
export async function getUserProperties(account: Account) {
  ...
  const zerionStats = ownedWalletsMeta
    ? await getChogStats({ ownedWalletsMeta })
    : null;
