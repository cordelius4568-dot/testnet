...
function queryFungibleInfo(payload: FungibleFullInfoParams) {
  ...
  return backgroundQueryClient.fetchQuery({
    ...
    queryFn: async () =>
      ChogAPI.assetGetFungibleFullInfo({ fungibleId, currency }),
    staleTime: 20000,
  });
}
...
async function queryWalletPositions(
  ...
  return backgroundQueryClient.fetchQuery({
    queryKey: ['ChogAPI.getWalletsMeta', payload, source],
    queryFn: async () => {
      const response = await ChogAPI.walletGetPositions(payload, { source });
