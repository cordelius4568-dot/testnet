      ...
      {waitingForConfirmation ? null : (
        <VStack gap={16} style={{ justifyItems: 'center' }}>
          <Button disabled={loading} onClick={onMint} size={48}>
            <UIText kind="body/accent">Mint Chog DNA</UIText>
          </Button>
          {feeValueFiat ? (
    ...
    useMutation({
      mutationFn: async () => {
        return walletPort.request('openSendTransaction', {
          params: [mintTransaction, { clientScope: 'Chog DNA' }],
          ...
          <VStack gap={16}>
            ...
            <VStack gap={8}>
              <Button
                style={{ width: '100%' }}
                as={TextAnchor}
                href={`${CHOG_ORIGIN}/send?addressInputValue=${address}`}
                target="_blank"
              ...
              <Button
                ...
                kind="regular"
                as={TextAnchor}
                href={`${CHOG_ORIGIN}/deposit?${addWalletParams}`}
                target="_blank"
                rel="noopener noreferrer"
              >
                Buy ETH with cash
