...
function replaceButtonLabel(textNode: Node, regex: RegExp) {
  const { textContent } = textNode;
  if (textContent) {
    textNode.textContent = textContent.replace(regex, 'Chog');
  }
}
...
function rewriteConnectButtons(
  ...
  function analyzeElement(element: HTMLElement) {
    ...
    if (element.dataset.replacementStep === 'text') {
       ...
       * This is to avoid mutating both Metamask and Injected buttons to Chog
       * and to avoid mutating Injected buttons when Chog button exitst
       */
      visitTextNodes(element, (textNode) => {
        const matchesChog = isReplacementMatch(textNode, zerionRe);
