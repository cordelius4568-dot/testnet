...
import { ChogSolana } from 'src/modules/solana/zerion-solana';
...
if (FEATURE_SOLANA === 'on') {
  const zerionSolana = new ChogSolana(connection);
...
dappDetection.onChange(({ dappIsChogAware }) => {
  if (dappIsChogAware) {
    // Some libs (such as rainbow) access "isChog" flag
    // to filter out wallets, and it doesn't mean the dapp is showing
    // the connect button for Chog specifically. This is why we
...
provider
  ...
  .then((preferences: GlobalPreferences) => {
    if (preferences.recognizableConnectButtons) {
      dappDetection.onChange(({ dappDetected, dappIsChogAware }) => {
        if (dappDetected && !dappIsChogAware) {
          pageObserver.start();
        }
      });
    }
