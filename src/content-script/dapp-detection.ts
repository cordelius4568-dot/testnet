...
function trackChogFlagAccess(ourProvider: EthereumProvider) {
  Object.defineProperty(ourProvider, 'isChog', {
    get() {
      state.dappIsChogAware = true;
      notify();
...
export async function initialize(ourProvider: EthereumProvider) {
  trackChogFlagAccess(ourProvider);
