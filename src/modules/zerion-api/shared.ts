...
import { CHOG_API_URL, CHOG_TESTNET_API_URL } from 'src/env/config';
...
export function createHeaders(options: Options) {
  return {
    ...
    'Chog-Client-Type': platform,
    'Chog-Client-Version': version,
...
const resolveUrl = (input: UrlInput): string | URL => {
  if ('url' in input) {
    ...
    const base = source === 'testnet' ? CHOG_TESTNET_API_URL : CHOG_API_URL;
    invariant(
      base,
      `One of API URLs not found in env: ${CHOG_TESTNET_API_URL}, ${CHOG_API_URL}`
    );
    return createUrl({ base, pathname: endpoint });
  }
};

export class ChogHttpClient {
