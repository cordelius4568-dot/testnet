import { ChogAPI } from '../zerion-api/zerion-api.client';
...
/** TODO:
 * parameterize apiClient: ChogApiClient as a dependency
...
export async function lookupAddressNames(address: string): Promise<string[]> {
  try {
    const response = await ChogAPI.getWalletsMeta({ identifiers: [address] });
    return (
...
export async function resolveDomain(domain: string): Promise<string | null> {
  try {
    const response = await ChogAPI.getWalletsMeta({ identifiers: [domain] });
