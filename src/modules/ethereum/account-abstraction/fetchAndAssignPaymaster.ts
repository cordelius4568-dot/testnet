...
import type { ChogApiClient } from 'src/modules/zerion-api/zerion-api-bare';
...
export function adjustedCheckEligibility(
  params: Parameters<ChogApiClient['paymasterCheckEligibility']>[0],
  { source, apiClient }: { source: NetworksSource; apiClient: ChogApiClient }
) {
  const txCopy = { ...params };
...
async function getPaymasterParams(
  incomingTransaction: IncomingTransaction,
  {
    ...
    apiClient: ChogApiClient;
  }
) {
  const transaction = normalizeTransactionProps(incomingTransaction);
  type Request = Parameters<ChogApiClient['getPaymasterParams']>[0];
